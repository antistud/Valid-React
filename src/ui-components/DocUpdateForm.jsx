...
import { getDoc } from "../graphql/queries";
import { updateDoc } from "../graphql/mutations";
const client = generateClient();
export default function DocUpdateForm(props) {
  const {
    id: idProp,
    doc: docModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = docRecord
      ? { ...initialValues, ...docRecord }
      : initialValues;
  ...
  const [docRecord, setDocRecord] = React.useState(docModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getDoc.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getDoc
        : docModelProp;
  ...
  React.useEffect(resetStateValues, [docRecord]);
  const validations = {
    title: [{ type: "Required" }],
    slug: [],
    body: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateDoc.replaceAll("__typename", ""),
            variables: {
              input: {
                id: docRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "DocUpdateForm")}
      {...rest}
    >
      <TextField
        label="Title"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || docModelProp)}
